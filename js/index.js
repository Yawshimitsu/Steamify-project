...
function renameCity() {
  ...
  const cityOldId = document.querySelector('#renameCity .listCities option:checked').value;
...
function addCity() {
  ...
  var postData = {
    name: newCityName, population: Math.floor(Math.random() * 100000)
  };
  fetch("https://avancera.app/cities/", {
    method: 'post',
    headers: {
...
function removeCity() {
  const deleteCityId = document.querySelector('#deleteCity .listCities option:checked').value;
...
function createChart(games) {
  ...
  prices.sort((a, b) => a.localeCompare(b, undefined, { numeric: true })).forEach(p => {
    amountOfGamesPerPrice[p] = amountOfGamesPerPrice[p] ? amountOfGamesPerPrice[p] + 1 : 1;
  })
